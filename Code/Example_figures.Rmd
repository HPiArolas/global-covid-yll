...
global_gender<-data.frame(Country=c("","WEIGHTED AVERAGE","AVERAGE"),Date=rep(NA,3),YLL.b_abs=rep(NA,3)
                  ...
                  ,Rate=c(NA,1.29,1.39),Rate_color=c(NA,plot_col[5],plot_col[5]))
...
global_age<-data.frame(Country=rep(c("","GLOBAL AVERAGE"),each=3),Age=rep(c("Under 55","55-75","75+"),2)
                  ,Proportion=c(NA,NA,NA,0.33,0.45,0.22),Continent=rep(NA,6),Continent_col=rep(NA,6))
...
flu_base_plot <-ggplot(Base2, aes(x=`Flu (med)`, y=Country)) +
    ...
    annotate(geom="text",x=1.1,y=42,label="0.777",size=3,col=plot_col2[2])+
    annotate(geom="text",x=0.35,y=42,label="0.596",size=3,col=plot_col2[3])+
    #average lines
    ...
    geom_vline(xintercept=mean(Base2$`Flu (max)`), linetype="solid", color=plot_col2[3], 
               size=1.25, alpha = 0.65)+
    theme_light() + ylab("")+ xlab("A. Ratio of years of life lost due to COVID-19 vs seasonal influenza") +
...
transport_base_plot <-ggplot(Base2, aes(x=Transport, y=Country)) +
    ...
    geom_vline(xintercept=mean(Base2$Transport) ,linetype="solid", color=plot_col2[1], 
               size=1.25, alpha = 0.65) +
    annotate(geom="text",x=0.8,y=42,label="0.620",size=3,col=plot_col2[1])+
    theme_light() + ylab("")+ xlab("B. Ratio of years of life lost due to COVID-19 vs transport") +
    theme(panel.border = element_blank(), axis.ticks.y = element_blank()
          , axis.text.y=element_text(colour="gray20"))
#transport_base_plot
```
...
heart_base_plot <-ggplot(Base2, aes(x=Heart, y=Country)) +
    ...
    geom_vline(xintercept=mean(Base2$Heart) ,linetype="solid", color=plot_col2[4], 
               size=1.25, alpha = 0.65) +
    annotate(geom="text",x=0.065,y=42,label="0.052",size=3,col=plot_col2[4])+
    theme_light() + ylab("")+ xlab("C. Ratio of years of life lost due to COVID-19 vs heart diseases") +
    theme(panel.border = element_blank(), axis.ticks.y = element_blank()
          , axis.text.y=element_text(colour="gray20"))
#heart_base_plot
```
...
excess_base_plot <-ggplot(Base2, aes(x=Excess, y=Country)) +
    ...
    geom_vline(xintercept=mean(Base2$Excess,na.rm=TRUE) ,linetype="solid", color=plot_col2[5], 
               size=1.25, alpha = 0.65) +
    annotate(geom="text",x=0.61,y=42,label="0.561",size=3,col="goldenrod4")+ #color too light as yellow#plot_col2[5])+
    theme_light() + ylab("")+ xlab("D. Ratio of years of life lost due to COVID-19 vs excess mortality") +
    theme(panel.border = element_blank(), axis.ticks.y = element_blank()
          , axis.text.y=element_text(colour="gray20"))
#excess_base_plot
```
...
age55_base_plot<-ggplot(age2, aes(fill=Age, y=Proportion, x=Country)) + 
    ...
    xlab("")+ylab("A.  Proportion of years of life lost by age group") +
    theme_bw() +
    ggtitle(" ") +
    theme(panel.border = element_blank(), axis.ticks.y = element_blank()
          , axis.text.y=element_text(colour="gray20")) +
...
tmp_mean_weight<-sum(gender2$Deaths/sum(gender2$Deaths)*gender2$YLL_rates_male_to_female,na.rm=TRUE)
...
gender_base_plot<-ggplot(gender2, aes(y=Country, x=Rate)) +
  ...
  annotate(geom="text",x=c(1.39+0.2,1.29+0.2),y=c(44,43),label=c("1.39","1.29"),size=3,col="gray20")+ 
...
```{r,echo=TRUE,eval=TRUE,warning = FALSE,message=FALSE}
gender_log<-gender_base_plot + scale_x_continuous(trans='log10',breaks = c(0.9,1,2,3,4)) + removeGridX()
gender_log
```
...
```{r,echo=TRUE,eval=TRUE,warning = FALSE,message=FALSE}
si1_base_plot <- ggplot(data=si1,aes(x=Diff.Days,y=YLL.b_abs)) + geom_point(alpha=0.5) +
...
si3_55 <- si3_55_base + ylab("Proportion of YLL for Age under 55") + ggtitle("Age under 55") +
  geom_text(data=age2_55 %>% filter(Country=="USA"|Country=="Brazil"|
                   Country=="Chile"|Country=="Italy"|Country=="Spain"|Country=="England"|
                   Country=="France"|Country=="Bangladesh"|Country=="Portugal"|Country=="Canada")
                   ,aes(label=Country),hjust=-0.0, vjust=0.0
                   ,position = position_jitter(width=0.01,height=0.01))
si3_55
```
