...
##Pull in days since first covid data
...
global_age<-data.frame(Country=rep(c("","GLOBAL AVERAGE"),each=3),Age=rep(c("Under 55","55-75","75+"),2)
                  ,Proportion=c(NA,NA,NA,0.241,0.422,0.337),Continent=rep(NA,6),Continent_col=rep(NA,6))
...
flu_base_plot <-ggplot(Base2, aes(x=`Flu (med)`, y=Country)) +
    ...
    annotate(geom="text",x=c(5.2,5.2),y=c(4,2),label=c("Median influenza year"
               ,"Maximum influenza year"),size=3,col=c("gray20")) +
    annotate(geom="text",x=1.2,y=42,label=paste(round(mean(Base2$`Flu (med)`),3)),size=3.5,col=plot_col2[2])+
    annotate(geom="text",x=0.32,y=42,label=paste(round(mean(Base2$`Flu (max)`),3)),size=3.5,col=plot_col2[3])+
    #average lines
...
transport_base_plot <-ggplot(Base2, aes(x=Transport, y=Country)) +
    ...
    geom_vline(xintercept=mean(Base2$Transport) ,linetype="solid", color=plot_col2[1], 
               size=1.25, alpha = 0.65) +
    annotate(geom="text",x=0.9,y=42,label=paste(round(mean(Base2$Transport),3)),size=3.5,col=plot_col2[1])+
    theme_light() + ylab("")+ 
...
heart_base_plot <-ggplot(Base2, aes(x=Heart, y=Country)) +
    ...
    geom_vline(xintercept=mean(Base2$Heart) ,linetype="solid", color=plot_col2[4], 
               size=1.25, alpha = 0.65) +
    annotate(geom="text",x=0.09,y=42,label=paste(round(mean(Base2$Heart),3)),size=3.5,col=plot_col2[4])+
    theme_light() + ylab("")+ 
...
excess_base_plot <-ggplot(Base2, aes(x=Excess, y=Country)) +
    ...
    geom_vline(xintercept=mean(Base2$Excess,na.rm=TRUE) ,linetype="solid", color=plot_col2[5], 
               size=1.25, alpha = 0.65) +
    annotate(geom="text",x=0.62,y=42,label=paste(round(mean(Base2$Excess,na.rm=TRUE),3)),size=3.5,col="goldenrod3")+ #color too light as yellow#plot_col2[5])+
    theme_light() + ylab("") + 
...
[2x2 panel saved as `Fig1.pdf` in Figures folder on DB.]{color="blue"}
...
age55_base_plot<-ggplot(age2, aes(fill=Age, y=Proportion, x=Country)) + 
    ...
    annotate(geom="text",x=rep(44,3),y=c(0.16,0.58,0.88),label=c("0.241","0.422","0.337"),size=3.5,col=c("gray50","white","white")) + #from S7 Table weighted proportions for Figure 2
    xlab("") + 
...
gender_base_plot<-ggplot(gender2, aes(y=Country, x=Rate)) +
  ...
  annotate(geom="text",x=c(1.9,1.85,0.66,1.4),y=c(44,43,3,3)
...
gender_base_plot2<-ggplot(gender2, aes(y=Country, x=Rate)) +
  ...
  geom_vline(xintercept=1,linetype="solid"
             ,color=viridis(n=1,alpha=1,begin=0.25,end=1,option="E"),size=1.25) +
  #add in global and weighted avg values
  annotate(geom="text",x=c(1.74,1.28,0.93,1.1),y=c(44,43,4,4)
...
[Adding groups under 55 and between 55-75, we get `r sum(0.284+0.421)`, or 2/3 (70%) of the total YLL comes from ages under 75.]{color="blue"}
...
```{r,echo=TRUE,eval=TRUE,warning = FALSE,message=FALSE}
si1_base_plot <- ggplot(data=si1,aes(x=Diff.Days,y=YLL.b_abs)) + geom_point(alpha=0.5) + xlim(26,170) +
...
si1_plot<-si1_base_plot + ylab("Years of life lost") +
  geom_text(data=si1 %>% filter(Country=="USA"|Country=="Brazil"|
                   Country=="Chile"|Country=="Italy"|Country=="Spain"|Country=="England"|
                   Country=="France"|Country=="Mexico"|Country=="Sweden"|Country=="Canada")
...
si_alpha<-0.5
##FLU
si2_flu_base <- ggplot(data=Base2,aes(x=Diff.Days,y=`Flu (med)`)) + xlim(26,170) +
...
si2_flu<- si2_flu_base + ylab("Ratio COVID YLL/Flu YLL") + 
  ...
  geom_text(data=Base2 %>% filter(Country=="USA"|Country=="Brazil"|
                   Country=="Chile"|Country=="Italy"|Country=="Spain"|Country=="England"|#Country=="France"|
...
si2_flu
##Transport
si2_transport_base <- ggplot(data=Base2,aes(x=Diff.Days,y=Transport)) + xlim(26,170) +
...
si2_transport
##Heart
si2_heart_base <- ggplot(data=Base2,aes(x=Diff.Days,y=Heart)) + xlim(26,170) +
...
si2_excess_base <- ggplot(data=si_excess_data,aes(x=Diff.Days,y=Excess)) + xlim(75,170) +
...
si2_excess<- si2_excess_base + ylab("Ratio COVID YLL/Excess mortality YLL") + 
  ...
  geom_text(data=si_excess_data #%>% filter(Country=="Norway"|
                   #Country=="Chile"|Country=="Italy"|
                   #Country=="Portugal"|Country=="Mexico"|Country=="Sweden"|Country=="Canada"|
                   #Country=="Taiwan"|Country=="South Africa"|Country=="Japan")
                   ,aes(label=Country),hjust=-0.0, vjust=0.0
                   ,position = position_jitter(width=0.01,height=0.01))
si2_excess
...
si2_logexcess<- si2_excess_base + scale_y_continuous(trans='log10') + ylab("Log scale ratio COVID YLL/Excess mortality YLL") +
                ggtitle("EXCESS MORTALITY") +
                geom_text(data=si_excess_data #%>% filter(Country=="USA"|
                   #Country=="Chile"|Country=="Italy"|
                   #Country=="Portugal"|Country=="Mexico"|Country=="Sweden"|Country=="Canada"|
                   #Country=="Taiwan"|Country=="South Africa"|Country=="Japan")
                   ,aes(label=Country),hjust=-0.0, vjust=0.0
